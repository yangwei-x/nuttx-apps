...
#define NOTE_MSG_SIZE CONFIG_EXAMPLES_TRACED_BUFFER_SIZE
...
static int create_socket(void)
{
  ...
  addr.sin_port        = HTONS(CONFIG_EXAMPLES_TRACED_IPPORT);
...
int main(int argc, char **argv, char **envp)
{
  ...
  server.sin_port        = HTONS(CONFIG_EXAMPLES_TRACED_IPPORT);
  ...
  while (true)
    {
      ...
      int note_bytes = read(note_fd, outbuf, NOTE_MSG_SIZE);
      ...
      if (metric_freq == CONFIG_EXAMPLES_TRACED_METRIC_PERIOD)
        {
          metric_freq = 0;
